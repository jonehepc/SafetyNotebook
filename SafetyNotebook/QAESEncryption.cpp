...
QByteArray QAESEncryption::RemovePadding(const QByteArray &rawText, QAESEncryption::PADDING padding) {
    ...
    auto padding_len = ret.at(ret.length() - 1);
    if (padding_len < 8) {
        bool valid = true;
        auto start = 0;
        switch (padding) {
            ...
            case PADDING::PKCS7:
            case PADDING::X923:
                for (int i = 1; i < padding_len - 1; i++) {
                    if (ret.at(ret.length() - 1 - i) != (padding == PADDING::PKCS7 ? padding_len : 0x00))
                        valid = false;
                }
                if (valid) {
                ...
                break;
            case PADDING::ISO:
                start = ret.lastIndexOf(0x80);
                for (int i = start; i < ret.length() - 1; i++) {
                    if (ret.at(start + 1) != 0x00) {
                        valid = false;
                        break;
                    }
                }
...
QByteArray QAESEncryption::decode(const QByteArray &rawText, const QByteArray &key, const QByteArray &iv) {
    ...
    return removePadding(ret, m_blocklen);
}

QByteArray QAESEncryption::removePadding(const QByteArray &rawText, int alignment) {
    QByteArray ret(rawText);
    auto padding_len = ret.at(ret.length() - 1);
    if (padding_len < alignment) {
        ...
        switch (m_padding) {
            ...
            case PADDING::X923:
                for (int i = 1; i < padding_len - 1; i++) {
                    if (ret.at(ret.length() - 1 - i) != (m_padding == PADDING::PKCS7 ? padding_len : 0x00))
                        valid = false;
                }
                if (valid) {
                    ret.remove(ret.length() - ret.at(ret.length() - 1), ret.at(ret.length() - 1));
